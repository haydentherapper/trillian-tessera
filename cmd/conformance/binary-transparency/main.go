...
// pkg:pypi/pandas@1.2.3
type LogEntry struct {
	PURL   string `json:"purl"`   // purl string, e.g. pkg:pypi/pandas@1.2.3
	Digest string `json:"digest"` // hex-encoded sha256 digest
}
...
func main() {
	...
	// Stores mapping between purl and sha256:digest
	...
	http.HandleFunc("POST /add", func(w http.ResponseWriter, r *http.Request) {
		...
		// Create entry
		var e LogEntry
		if json.Unmarshal(b, &e); err != nil {
			w.WriteHeader(http.StatusBadRequest)
			_, _ = w.Write([]byte(err.Error()))
			return
		}

		_, ok := lookupMap[e.PURL]
		if ok {
			w.WriteHeader(http.StatusConflict)
			_, _ = w.Write([]byte("entry exists"))
			return
		}

		// purl@@sha256:digest
		leafData := fmt.Sprintf("%s@@sha256:%s", e.PURL, e.Digest)

		idx, err := addFn(r.Context(), tessera.NewEntry([]byte(leafData)))()
		if err != nil {
			w.WriteHeader(http.StatusInternalServerError)
			_, _ = w.Write([]byte(err.Error()))
			return
		...
		// add digest lookup to map
		lookupMap[e.PURL] = fmt.Sprintf("sha256:%s", e.Digest)
	})

	// we probably shouldn't have this, unless a language ecosystem has reproducible builds
	http.HandleFunc("GET /lookup/{entry}", func(w http.ResponseWriter, r *http.Request) {
		lookupKey := r.PathValue("entry")
		if lookupKey == "" {
			w.WriteHeader(http.StatusBadRequest)
			_, _ = w.Write([]byte("lookup entry key missing"))
			return
		}

		// TODO: verify purl spec adherence

		val, ok := lookupMap[lookupKey]
		if !ok {
		...
		if _, err = w.Write([]byte(val)); err != nil {
			klog.Errorf("/lookup/%s: %v", lookupKey, err)
			return
		}
	})
